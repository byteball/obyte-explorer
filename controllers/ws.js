...
function start(data) {
	...
	if (data.type === 'last') {
		...
		SELECT unit FROM outputs WHERE address = ? LIMIT 0,1", [data.address, data.address, data.address], function(rows) {
			if(rows.length || 1) {
				address.getAddressInfo(data.address, data.filter || {}, function(objTransactions, unspent, objBalance, end, definition, newLastInputsROWID, newLastOutputsROWID, objStateVars, arrAaResponses) {
